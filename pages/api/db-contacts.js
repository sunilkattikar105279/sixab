...
function noDb(res) { return res.status(500).json({ error: 'NEXT_PUBLIC_SUPABASE_URL / SUPABASE_SERVICE_ROLE_KEY not set in Vercel env vars' }) }
  ...
  if (req.method === 'GET') {
    ...
    let qs = `user_id=eq.${uid}&order=updated_at.desc&limit=${limit}`
    if (stage) qs += `&stage=eq.${stage}`
    const r = await fetch(`${SB_URL}/rest/v1/crm_contacts?${qs}`, { headers: { apikey: SB_SVC, Authorization: `Bearer ${SB_SVC}` } })
    const contacts = await r.json()
    return res.status(200).json({ contacts: Array.isArray(contacts) ? contacts : [] })
  }
  if (req.method === 'POST') {
    const data = await sbPost('crm_contacts', { ...req.body, user_id: uid })
    return res.status(201).json({ contact: data })
  }
  if (req.method === 'PUT') {
    const { id, ...updates } = req.body
    const data = await sbPatch('crm_contacts', id, uid, updates)
    return res.status(200).json({ contact: data })
  }
  if (req.method === 'DELETE') {
    await sbDelete('crm_contacts', req.query.id, uid)
