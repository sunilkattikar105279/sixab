...
// Conversational agent: describe → build → refine → deploy
import Head from "next/head"
import SixxabNav from "../components/SixxabNav"
import { useState, useEffect, useRef } from "react"

const N = "#0A0E1A", AMBER = "#EF9F27", CHALK = "#F5F5F0"
const GREEN = "#1D9E75", PURPLE = "#7C3AED"

// ── Saved projects ────────────────────────────────────────────────────────────
const STORE = "sixxab_wb_projects"
...
// ── Quick-start prompts ───────────────────────────────────────────────────────
const STARTERS = [
  { icon:"🏢", label:"Tech consulting", prompt:"Build a professional website for BigTech Consulting — enterprise AI transformation and IT strategy firm in Dallas TX. Services: Digital Transformation, Cloud Migration, Cybersecurity, Data Analytics. Corporate blue palette, authoritative design." },
  { icon:"🔧", label:"HVAC business",   prompt:"Build a website for Dallas Pro HVAC — local AC and heating company in DFW. Emergency contact button, services grid (AC repair, installation, maintenance), trust badges, 5-star reviews, strong local SEO." },
  { icon:"⚖️", label:"Law firm",        prompt:"Create a website for Morrison & Associates — Dallas family law and estate planning firm. Dark navy with gold accents, professional trust-first design. Practice areas, attorney bio, consultation booking, FAQ, testimonials." },
  { icon:"🏠", label:"Real estate",     prompt:"Build a website for Sarah Johnson Realty — luxury Dallas real estate agent. Warm elegant design. Buyer/seller services, featured listings section, market reports, client testimonials, booking CTA." },
  { icon:"💪", label:"Personal trainer",prompt:"Create a website for Jake Morrison Fitness — personal training in Dallas. Bold energetic design. Programs: weight loss, muscle building, athlete training. Before/after section, pricing, free consultation CTA." },
  { icon:"🍕", label:"Restaurant",      prompt:"Build a website for Rosario's Kitchen — authentic Italian restaurant in Dallas. Warm inviting design. Menu highlights, reservation CTA, chef story, gallery, location and hours." },
]
...
export default function WebsiteBuilder() {
  ...
  // ── Project management ────────────────────────────────────────────────────
  function newProject() {
    setActiveId(null); setMessages([]); setHtml("")
    setInput(""); setDeployResult(null)
  }

  function upsertProject(newHtml, msgs) {
    const title = newHtml.match(/<title>(.*?)<\/title>/i)?.[1]?.replace(/\s*[|·-].*$/, "").trim() || "Website"
    const proj  = { id: activeId || Date.now(), title, html: newHtml, updatedAt: new Date().toISOString() }
    const updated = activeId
      ? projects.map(p => p.id === activeId ? proj : p)
      : [proj, ...projects].slice(0, 20)
    setProjects(updated)
  ...
  function loadProject(p) {
    setActiveId(p.id); setHtml(p.html); setMessages([])
    setDeployResult(null); setShowProjects(false)
  ...
  async function send(overridePrompt) {
    const msg = (overridePrompt || input).trim()
    ...
    try {
      const res = await fetch("/api/website-agent", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ prompt: msg, html: html || undefined }),
      })
  ...
  async function deploy() {
    ...
    try {
      const title = html.match(/<title>(.*?)<\/title>/i)?.[1]?.replace(/\s*[|·-].*$/,"").trim() || "sixxab-site"
      const slug  = title.toLowerCase().replace(/[^a-z0-9]/g, "-").replace(/-+/g, "-").slice(0, 52)
      const r = await fetch("/api/deploy-vercel", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ html, projectName: slug }),
      })
      const d = await r.json()
      setDeployResult(d)
      if (d.deployed)    showToast("Deploying! Live in ~60 seconds 🚀")
      else if (d.needsSetup) showToast("Add VERCEL_TOKEN to Vercel env vars", false)
  ...
  function download() {
    if (!html) return
    const title = html.match(/<title>(.*?)<\/title>/i)?.[1] || "website"
  ...
  return (
    <>
      <Head>
        <title>SIXXAB AI — Website Builder</title>
        <meta name="description" content="Describe your business. AI builds your complete website. Refine with conversation. Deploy to Vercel in one click."/>
      </Head>
      ...
      <style>{`
        * { box-sizing: border-box; margin: 0; padding: 0 }
        ...
        .chat-inp {
          ...
          border-radius: 12px; padding: 11px 15px; font-size: 14px; color: ${CHALK};
          font-family: inherit; outline: none; resize: none; line-height: 1.5;
          transition: border .15s; max-height: 130px; min-height: 44px;
        }
        ...
        /* Suggestion pill */
        .pill {
          padding: 5px 12px; border-radius: 18px; border: 1px solid rgba(255,255,255,.1);
          background: rgba(255,255,255,.04); color: rgba(245,245,240,.6);
          font-size: 12px; cursor: pointer; font-family: inherit; transition: all .14s;
          text-align: left; white-space: nowrap;
        }
        .pill:hover { background: rgba(239,159,39,.12); border-color: rgba(239,159,39,.4); color: ${AMBER} }

        /* View tabs */
        .vtab { padding: 5px 13px; border-radius: 7px; font-size: 12px; font-weight: 500;
        ...
        /* Bubble */
        .bubble-user { background: rgba(239,159,39,.13); border: 1px solid rgba(239,159,39,.22);
          border-radius: 14px 14px 3px 14px; padding: 9px 13px; max-width: 88%;
          font-size: 13.5px; line-height: 1.6; white-space: pre-wrap; word-break: break-word }
        .bubble-ai { background: rgba(255,255,255,.05); border: 1px solid rgba(255,255,255,.09);
          border-radius: 14px 14px 14px 3px; padding: 9px 13px; max-width: 92%;
          font-size: 13.5px; line-height: 1.6; color: rgba(245,245,240,.85) }
        ...
        @media(max-width:800px) {
          ...
          .main-panel { height: 50vh !important }
        }
      `}</style>
      ...
      <div style={{ display:"flex", height:"calc(100vh - 52px)" }}>
        ...
        <div className="sidebar" style={{
          ...
          borderRight:"1px solid rgba(255,255,255,.08)", background:"#0D1117",
        }}>
          ...
          <div style={{
            ...
            <div style={{ display:"flex", gap:6 }}>
              <button onClick={() => setShowProjects(s => !s)}
                style={{ padding:"4px 10px", borderRadius:7, background:"rgba(255,255,255,.06)",
                  border:"1px solid rgba(255,255,255,.1)", fontSize:11.5,
                  color:"rgba(245,245,240,.5)", cursor:"pointer", fontFamily:"inherit" }}>
                {projects.length} saved
              </button>
              <button onClick={newProject}
                style={{ padding:"4px 12px", borderRadius:7, background:AMBER,
                  color:N, border:"none", fontSize:12, fontWeight:700, cursor:"pointer", fontFamily:"inherit" }}>
                + New
              </button>
            </div>
          </div>
          ...
          {showProjects && (
            <div style={{ borderBottom:"1px solid rgba(255,255,255,.07)", maxHeight:180, overflowY:"auto", flexShrink:0 }}>
              {projects.length === 0 ? (
                <div style={{ padding:"14px", fontSize:12.5, color:"rgba(245,245,240,.3)" }}>
                ...
                <div key={p.id}
                  ...
                  style={{
                    ...
                    background: activeId===p.id ? "rgba(239,159,39,.08)" : "transparent",
                    borderLeft: activeId===p.id ? `2px solid ${AMBER}` : "2px solid transparent",
                ...
                >
                  <i className="ti ti-file-code" style={{ fontSize:14, color:"rgba(245,245,240,.3)", flexShrink:0 }} aria-hidden="true"/>
                  <div style={{ flex:1, minWidth:0 }}>
                    <div style={{ fontSize:12.5, fontWeight:500, color:CHALK, overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>
                      {p.title}
                    </div>
                    <div style={{ fontSize:10.5, color:"rgba(245,245,240,.25)" }}>
                      {new Date(p.updatedAt).toLocaleDateString()}
                    </div>
                  </div>
                  <button
                    onClick={e => { e.stopPropagation(); deleteProject(p.id) }}
                    style={{ background:"none", border:"none", cursor:"pointer", color:"rgba(245,245,240,.2)", fontSize:16, padding:"0 3px" }}>
                    ×
                  </button>
                </div>
              ))}
          ...
          <div style={{ flex:1, overflowY:"auto", padding:"14px" }}>
            {messages.length === 0 ? (
              ...
              <div>
                <p style={{ fontSize:13.5, fontWeight:600, color:CHALK, marginBottom:4 }}>
                  Describe your website
                </p>
                <p style={{ fontSize:12.5, color:"rgba(245,245,240,.4)", lineHeight:1.65, marginBottom:18 }}>
                  Tell me your business name, what you do and the vibe you want.
                  I'll build a complete, production-ready website.
                </p>
                <div style={{ fontSize:10.5, fontWeight:700, color:"rgba(245,245,240,.2)", textTransform:"uppercase", letterSpacing:".1em", marginBottom:10 }}>
                  Quick start
                </div>
                  ...
                  {STARTERS.map((s, i) => (
                    <button key={i} onClick={() => send(s.prompt)}
                      style={{
                        ...
                        fontFamily:"inherit", textAlign:"left", transition:"all .14s",
                      }}
                    ...
                    >
                      ...
                      <div>
                        ...
                        <div style={{ fontSize:11, color:"rgba(245,245,240,.3)", lineHeight:1.4 }}>
                          {s.prompt.slice(0, 70)}…
                        </div>
                      </div>
                    </button>
              ...
              <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
                {messages.map((m, i) => (
                  <div key={i} className="fu" style={{ animationDelay:`${i * 0.03}s` }}>
                    {m.role === "user" ? (
                      ...
                      <div style={{ display:"flex", gap:8, alignItems:"flex-start" }}>
                        ...
                        <div style={{ flex:1, minWidth:0 }}>
                          ...
                          {/* Message text */}
                          <div className={`bubble-ai${m.error?" bubble-err":""}`}>
                            {m.content}
                          </div>
                          {/* Suggestions */}
                          {m.suggestions?.length > 0 && (
                            <div style={{ marginTop:8, display:"flex", flexWrap:"wrap", gap:5 }}>
                              {m.suggestions.map((s, j) => (
                                <button key={j} className="pill" onClick={() => send(s)}>{s}</button>
                              ))}
                            </div>
                          )}
                        </div>
          ...
          <div style={{ padding:"12px 14px", borderTop:"1px solid rgba(255,255,255,.07)", flexShrink:0 }}>
            <div style={{ display:"flex", gap:8, alignItems:"flex-end" }}>
              ...
              <button
                ...
                style={{
                  ...
                  background: input.trim() && !loading ? AMBER : "rgba(255,255,255,.06)",
            ...
            <div style={{ fontSize:10.5, color:"rgba(245,245,240,.2)", marginTop:5 }}>
        ...
        <div id="wb-preview-panel" className="main-panel" style={{ flex:1, display:"flex", flexDirection:"column", minWidth:0, background:"#111827" }}>
          ...
          <div style={{
            ...
            padding:"0 14px", gap:8, borderBottom:"1px solid rgba(255,255,255,.07)", background:"#0D1117",
          }}>
            {/* View toggle */}
            ...
            {hasHtml && (
              <div style={{ fontSize:11.5, color:"rgba(245,245,240,.3)", marginLeft:4 }}>
          ...
          {deployResult && (
            <div style={{
              ...
              background: deployResult.deployed ? "rgba(29,158,117,.1)" : deployResult.needsSetup ? "rgba(239,159,39,.08)" : "rgba(220,38,38,.1)",
              borderBottom: `1px solid ${deployResult.deployed?"rgba(29,158,117,.2)":deployResult.needsSetup?"rgba(239,159,39,.2)":"rgba(220,38,38,.2)"}`,
            }}>
              {deployResult.deployed ? (
                <>
                  ...
                  <a href={deployResult.url} target="_blank" rel="noopener noreferrer"
                    style={{ fontSize:12.5, color:GREEN, fontWeight:700, textDecoration:"none", marginLeft:"auto" }}>
                    {deployResult.url} ↗
                  </a>
                </>
              ) : deployResult.needsSetup ? (
                <>
                  <span style={{ fontSize:12.5, color:AMBER }}>⚙️ Add VERCEL_TOKEN in Vercel → Project → Settings → Environment Variables to enable one-click deploy</span>
                  <button onClick={download}
                    style={{ marginLeft:"auto", padding:"5px 12px", borderRadius:7, background:AMBER, color:N, border:"none", fontSize:12, fontWeight:600, cursor:"pointer", fontFamily:"inherit" }}>
                    Download HTML →
                  </button>
                </>
              ) : (
              ...
              <button onClick={() => setDeployResult(null)}
                style={{ background:"none", border:"none", cursor:"pointer", color:"rgba(245,245,240,.3)", fontSize:18, padding:"0 4px", marginLeft: deployResult.deployed ? 0 : 0 }}>
                ×
              </button>
            </div>
          )}
          ...
          <div style={{ flex:1, overflow:"hidden", position:"relative" }}>
            {!hasHtml ? (
              ...
              /* Code view */
              <div style={{ height:"100%", overflowY:"auto", padding:16, fontFamily:"'DM Mono',monospace", fontSize:11.5, lineHeight:1.65, color:"rgba(245,245,240,.6)", whiteSpace:"pre-wrap", wordBreak:"break-word", background:"#0D1117" }}>
                {html}
              </div>
            ) : view === "preview" ? (
              /* Full preview */
              ...
              <div style={{ display:"flex", height:"100%" }}>
                ...
                />
                {/* Quick edits panel */}
                <div style={{ width:240, overflowY:"auto", padding:14, flexShrink:0 }}>
                  <div style={{ fontSize:10.5, fontWeight:700, color:"rgba(245,245,240,.25)", textTransform:"uppercase", letterSpacing:".1em", marginBottom:10 }}>
                    Quick edits
                  </div>
                  <div style={{ display:"flex", flexDirection:"column", gap:5 }}>
                    {[
                      "Change to dark color scheme",
                      "Add a pricing section with 3 tiers",
                      "Make the hero more impactful",
                      "Add more social proof",
                      "Improve the contact section",
                      "Add FAQ with 6 questions",
                      "Change to a warmer color palette",
                      "Add team member bios",
                      "Improve mobile layout",
                      "Add a sticky announcement bar at top",
                    ].map((s, i) => (
                  ...
                  <div style={{ marginTop:16, paddingTop:14, borderTop:"1px solid rgba(255,255,255,.06)" }}>
                    ...
                    {[["📅 Schedule posts","/calendar"],["💬 Generate leads","/leads"],["📊 Social pages","/social"],["📈 Analytics","/analytics"]].map(([l,h]) => (
